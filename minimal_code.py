"""
minimal_code.py — Minimal Quest → Python pipeline (no PyBullet)

Functionalities
---------------
  1. Receive headset pose + hand poses from Unity (ZMQ port 5570)
  2. Camera feed from Unity (ZMQ port 5560) + ArUco marker detection
  3. Coordinate transform anchored to marker 100 (world frame)
     + pegboard detection via marker 101
     Reanchor: ENTER key in OpenCV window, or Unity sends
       {"type": "reanchor"} / {"type": "reanchor_pegboard"} to port 5014
  4. Open3D visualization with coordinate frames

Keys (OpenCV window must be focused)
-------------------------------------
  ENTER = lock marker 100 as world frame / lock marker 101 as pegboard
  ESC   = quit

Usage
-----
  python minimal_code.py
  python minimal_code.py --quest-ip 192.168.50.201
"""
...
class _ArUcoWorker:
    """Background thread: polls camera feed and runs ArUco detection."""
...
class _SceneVis:
    ...
    def __init__(self, title: str, width: int = 1000, height: int = 680):
        ...
        # Anchor frame: shows tracking-space origin in world coordinates
        self._anchor_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.15)
        self._anchor_frame.transform(self._hidden_T())
        self.vis.add_geometry(self._anchor_frame)
        self._anchor_T = self._hidden_T()

        # Pegboard frame + sphere (green)
    ...
    def _update_frustum(self, attr: str, T: np.ndarray | None, color,
        ...
        if existing is None:
            setattr(self, attr, new_fr)
            self.vis.add_geometry(new_fr)
        else:
            existing.points = new_fr.points
            existing.lines  = new_fr.lines
            existing.colors = new_fr.colors
            self.vis.update_geometry(existing)

    def update_cam_frustum(self, T, w=640, h=480, fx=400., fy=400., cx=320., cy=240.):
        self._update_frustum("_cam_frustum", T, [0.2, 1.0, 0.3], w, h, fx, fy, cx, cy)

    def update_head(self, T, w=640, h=480, fx=400., fy=400., cx=320., cy=240.):
        self._update_frustum("_head_frustum", T, [1.0, 0.1, 0.9], w, h, fx, fy, cx, cy)

    def update_anchor(self, T: np.ndarray | None):
        T_new = T if T is not None else self._hidden_T()
        self._anchor_frame.transform(T_new @ np.linalg.inv(self._anchor_T))
        self._anchor_T = T_new
        self.vis.update_geometry(self._anchor_frame)
...
def run(quest_ip: str, anchor_marker_id: int, pegboard_marker_id: int,
    ...
    print(f"  ENTER with marker #{anchor_marker_id} visible → lock world frame")
    print(f"  ENTER with marker #{pegboard_marker_id} visible → lock pegboard")
    print("  ESC = quit\n")
...
def main():
    ap = argparse.ArgumentParser(
        description="Minimal Quest pipeline — marker 100 world frame, no PyBullet",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ap.add_argument("--quest-ip",        default=cfg.UNITY_IP)
    ap.add_argument("--anchor-marker",   type=int, default=100,
                    help="ArUco marker ID for world frame anchor")
