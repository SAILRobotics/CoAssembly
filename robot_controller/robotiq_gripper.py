...
class RobotiqGripper:
    ...
    def move_async(self, position: int, speed: int, force: int, callback=None) -> threading.Thread:
        """
        Non-blocking version of move_and_wait_for_pos.
        Starts a background thread to move the gripper and monitor completion.
        :param position: Target position (0-255)
        :param speed: Motion speed (0-255)
        :param force: Gripping force (0-255)
        :param callback: Optional function(result_pos: int, obj_status: ObjectStatus)
                        called when motion completes.
        :return: The thread object so caller can join() if needed.
        """
        ...
        def _worker():
            try:
                ...
                # Monitor motion until stopped
                cur_obj = self._get_var(self.OBJ)
                while RobotiqGripper.ObjectStatus(cur_obj) == RobotiqGripper.ObjectStatus.MOVING:
                    time.sleep(0.02)
                    cur_obj = self._get_var(self.OBJ)

                # Read final position and object status
                final_pos = self._get_var(self.POS)
                final_obj = RobotiqGripper.ObjectStatus(cur_obj)

                if callback:
                    callback(final_pos, final_obj)

            except Exception as e:
                print(f"[move_async] Error: {e}")
