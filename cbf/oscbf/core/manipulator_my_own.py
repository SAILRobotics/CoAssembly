...
try:
    ...
    from ur_collision_model import (  # direct import when run from oscbf/core/
        ur_collision_data,
        ur_self_collision_data,
        ur_base_self_collision_data,
        make_collision_data,
...
class Manipulator:
    ...
    def __init__(
        ...
        # ✨ NEW: precompute base transform (world -> joint0 parent)
        if self.base_pos is None or self.base_orn is None:
            base_T = np.eye(4, dtype=float)
        else:
            pos_np = np.asarray(self.base_pos, dtype=float).reshape(3,)
            orn_np = np.asarray(self.base_orn)
            if orn_np.shape == (3, 3):
                R_base = orn_np
            else:
                # assume xyzw quaternion
                assert orn_np.shape == (4,)
                R_base = quat_xyzw_to_matrix(orn_np)
            base_T = create_transform_numpy(R_base, pos_np)
        self.base_T = tuplify(base_T)
    ...
    def from_urdf(
        ...
        base_self_collision_data: Optional[dict] = None,
         # ✨ NEW:
        base_pos: Optional[ArrayLike] = None,
        base_orn: Optional[ArrayLike] = None,
        ...
        # ✨ NEW: if not provided, ignore URDF base and use identity,
        # keeping backward compatibility
        if base_pos is not None:
            base_pos = tuplify(np.asarray(base_pos).reshape(3,))
        else:
            base_pos = None

        if base_orn is not None:
            base_orn = tuplify(np.asarray(base_orn))
        else:
            base_orn = None
    ...
    def joint_to_world_transforms(self, q: Array) -> Array:
        ...
        # ✨ NEW: apply base transform
        base_T = jnp.asarray(self.base_T)
        cumulative_transforms = base_T @ cumulative_transforms
...
def load_ur10e(
    ...
    """Create a Manipulator object for the UR10e.

    Args:
        urdf_filename: Path to the UR10e URDF. If None, we try to find it under assets.
        ee_offset: Optional 4x4 transform from last joint frame to TCP.
                   If None, identity is used (TCP = flange frame).

    Returns:
        Manipulator: UR10e manipulator object.
    """
    if urdf_filename is None:
        # adjust this to wherever you store the ur10e.urdf in your assets
        urdf_filename = find_assets_dir() + "ur10e/ur10e.urdf"
    ...
    return Manipulator.from_urdf(
        ...
        ee_offset=ee_offset,
        collision_data=ur_collision_data,
        self_collision_data=ur_self_collision_data,
        base_self_collision_data=ur_base_self_collision_data,
    )

def quat_xyzw_to_matrix(q: ArrayLike) -> np.ndarray:
...
def load_ur10e_for_world(
    ...
    return Manipulator.from_urdf(
        urdf_filename=urdf_filename,
        ee_offset=ee_offset,
        collision_data=make_collision_data(with_gripper=with_gripper),
        self_collision_data=ur_self_collision_data,
        base_self_collision_data=ur_base_self_collision_data,
