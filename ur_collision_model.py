...
def visualize(q=None, T_world_base=None, with_gripper: bool = False) -> None:
    ...
    def get_q():
        with _lock:
            return _q.copy(), _with_gripper[0]
    ...
    def _sphere_wireframe(centers_radii, n=16):
        theta = np.linspace(0, 2 * np.pi, n, endpoint=False)
        c, s  = np.cos(theta), np.sin(theta)
        z     = np.zeros(n)
        pts, lns, off = [], [], 0
        for (ctr, r) in centers_radii:
        ...
        ls.colors = o3d.utility.Vector3dVector([[0.2, 1.0, 0.4]] * len(lns))
        return ls

    def _compute_sphere_world(poses, grip):
        """Transform collision spheres to world frame using PyBullet FK poses."""
        cdata  = make_collision_data(with_gripper=grip)
        result = []
        for link_i, (lpos, lrad) in enumerate(zip(cdata["positions"], cdata["radii"])):
            T = poses[link_i + 1]   # poses[0]=base, [1..6]=arm links
            for lp, r in zip(lpos, lrad):
                world = T[:3, :3] @ np.array(lp) + T[:3, 3]
                result.append((world, r))
        return result
        ...
        with dpg.window(label="Collision Model — Joints", tag="win",
            ...
            dpg.add_checkbox(label="With gripper", tag="grip_cb",
                             ...
                             callback=lambda: set_q(
                                 np.deg2rad([dpg.get_value(f"j{k}") for k in range(6)]),
                                 dpg.get_value("grip_cb")))
        dpg.create_viewport(title="Joints", width=360, height=320)
        dpg.setup_dearpygui()
        dpg.show_viewport()
    ...
    while vis.poll_events():
        ...
        if is_dirty:
            ...
            if spheres:
                new_ls = _sphere_wireframe(spheres)
