"""
main_hand_m10.py — Same as main_hand.py but uses ArUco marker 10 as the sole
world-frame anchor AND PyBullet scene origin.

Workflow
--------
  1. Hold marker 10 visible → press ENTER to lock the world frame.
     PyBullet scene is placed at the locked pose immediately.
  2. Hold marker 101 visible → press ENTER to lock the pegboard pose.
     Marker 10 does NOT need to be visible at this step.
  3. Press ENTER again at any time to re-lock either (whichever marker is visible).

Keys (OpenCV window must be focused)
--------------------------------------
  ENTER = lock / relock (handles marker 10 and/or 101 independently)
  ESC   = quit

Usage
-----
  python main_hand_m10.py
  python main_hand_m10.py --quest-ip 192.168.50.201 --hand-port 5570
"""
...
# =============================================================================
# ArUco pose estimator — marker 10 (anchor) + marker 101 (pegboard)
# =============================================================================
...
class _WorldAnchor:
    ...
    def lock(self, T_cam_anchor: np.ndarray,
        ...
        """Lock world frame to marker 10 (anchor). Returns True on success."""
        ...
        print(f"[Anchor] Locked to marker 10 ({src}).")
        return True
    ...
    def update_pegboard_from_tracking(self, cam_T: np.ndarray,
        ...
        Marker 10 does NOT need to be visible — uses the locked _T_wt instead.
        """
...
def run(quest_ip: str, anchor_marker_id: int, pegboard_marker_id: int,
    ...
    """
    anchor_marker_id  : ArUco marker that defines world frame + PyBullet origin (default 10)
    ...
    if _PYBULLET_AVAILABLE:
        ...
        if _calib_dir.exists():
            try:
                ...
                print(f"[main_hand_m10] PyBullet scene failed to build: {e}")
                pb_scene = None
        else:
            print(f"[main_hand_m10] Calibration dir not found: {_calib_dir}")
    ...
    # Cubes around the anchor marker (world origin = marker 10)
        ...
        while True:
            ...
            if key == 27:
                ...
                if cam.camera_T is None:
                    ...
                    # ── Phase A: Lock/relock world frame to marker 10 ─────────
                    if anchor_ok:
                        if anchor.locked:
                    ...
                    # Marker 10 does NOT need to be visible here.
...
def main():
    ap = argparse.ArgumentParser(
        description="Quest passthrough hand tracking — marker 10 as world+scene origin",
