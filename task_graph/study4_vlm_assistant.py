"""Study 4 VLM assistant with condition-controlled task-state access.
...
`BEARING_ROW1`..`BEARING_ROW4`, `PIN_ROW1`..`PIN_ROW4`, and
`SCREW_ROW1`..`SCREW_ROW4` group physically co-located/interchangeable items
without discarding the user's row reference. `BEARING` and `PIN` are used only
when no row is specified. `ROW1_KIT`..`ROW4_KIT` identify the four graspable
...
def _print_console_block(label: str, text: str) -> None:
    """Mirror VLM activity as one filterable terminal line per event."""
    kind = str(label).upper().removeprefix("VLM ").replace(" ", "_")
    payload = " ".join(str(text).split())
    # Full prompts/raw model output remain in the assistant CSV and GUI. Keep
    # the live console readable even when a model returns a long JSON envelope.
    if len(payload) > 800:
        payload = payload[:797] + "..."
    print(f"[VLM   ][{kind}] {payload}", flush=True)
...
class _VLMWorker:
    ...
    def start(self, load_model_in_current_thread: bool = False) -> None:
        self._running = True
        if load_model_in_current_thread:
            ...
            try:
                ...
                self._result_queue.put(
                    ("error", f"Model load failed: {error}", None))
                self._running = False
                return
        ...
        while self._running:
            ...
            try:
                answer = self._infer(
                    ...
                    # Intent grounding gets only the explicit structured
                    # discourse context assembled by submit_question(). Feeding
                    # the raw chat history as well caused stale attributes such
                    # as RIGHT to leak into a new Row 1 stand request.
                    use_history=(result_kind == "answer"),
                )
                with self._history_lock:
...
class VLMAssistant:
    ...
    def __init__(
        ...
        study4_condition: str | None = None,
    ) -> None:
        self.dpg = dpg
        self._model_name = model_name
        self._study4_condition = study4_condition
        ...
        if study4_condition == "language":
            self._system_prompt += (
                "\nSTUDY CONDITION: Ground the user's free-form object reference from "
            ...
            self._system_prompt += (
                ...
                "predict are relevant to that focused step. If several remain "
                "plausible, keep target AMBIGUOUS and return the reduced candidate "
                "set; do not guess one. If exactly one remains, return that exact "
                "label. The interface will visualize your candidate set and current "
                "prediction.\n")
            self._system_prompt += (
                "For this condition, when the user asks generically which "
                "physical parts/tools are needed for the focused step, return "
                "`part_reference`: use one exact target if you predict one object, "
                "or target `AMBIGUOUS` with every predicted relevant object in "
                "`candidates`. This condition-specific rule overrides the generic "
                "`step_items_request` rule below because "
                "the study measures the model's own candidate predictions.\n")
    ...
    def notify_graph_event(self, event_label: str, state_summary: str) -> None:
        ...
        if self._study4_condition == "language":
            state_summary = "Task-state details are intentionally withheld in this condition."
    ...
    def set_focused_step(self, focused_text: str) -> None:
        ...
        self._focused_step_available = bool(str(focused_text).strip())
        if self._study4_condition == "language":
            self._worker.set_focused_step(
                "Focused-step context is intentionally unavailable in this condition.")
        else:
            self._worker.set_focused_step(focused_text)

    def set_pending_fetch(self, description: str | None) -> None:
        """Keep the task-graph fetch-confirmation API compatible."""
    ...
    def submit_question(self, text: str) -> bool:
        ...
        if self._last_resolved_part_label:
            reference_context = (
                ...
                "An utterance that explicitly names a row, category, side, "
                "color, size, or position is a new `direct` reference unless "
                "it also uses a genuine anaphor such as it, that, or the same "
                "one. Never inherit an omitted side from the previous object. "
                "A plural category request is not `same_as_previous`.\n\n")
...
- A row-qualified stand request that does not explicitly say left or right is
  ambiguous between that row's LEFT and RIGHT stand. Return both exact stand
  labels as candidates; never inherit a side from conversation history. A
  plural request such as "gear stands for Row 1" also returns both stands.
- Use `step_items_request` when the user asks generically for physical items
  needed by the currently selected step without naming a physical object, such
  as "the part for this step", "what tools do I need?", or "what do I need?"
  Set `item_scope` to `parts`, `tools`, or `all` according to the request. Leave `target`,
  and `candidates` empty. A generic singular word such as "part" does
...
- For generic tool requests, use `step_items_request` with `item_scope`=`tools`.
  Use `part_action`=`status`
...
- Do not use `step_items_request` after the user names a particular tool, bit,
...
- Preserve the physical object category explicitly named by the user. A
  screwdriver/driver request must resolve to a screwdriver label, never to an
  insert such as H5_HEX_BIT, T25_TORX_BIT, or H3_HEX_BIT. Rows 1, 2, and 3 use
  `BIT_SCREWDRIVER`; Row 4 uses `PHILLIPS_SCREWDRIVER`. Resolve to H5_HEX_BIT,
  T25_TORX_BIT, or H3_HEX_BIT only when the user asks for a bit or insert.
- A request naming an actual object category—such as "the gear necessary for
  this step"—is a `part_reference`, not a generic step-items request. Use the focused
...
- "Other than the bearing, what other part do I need for this step?" ->
  {{"intent":"step_items_request","item_scope":"parts","part_action":"reference","step_scope":"selected_step","target":"","candidates":[],"exclude_labels":["BEARING"]}}
- "Can you highlight the other part necessary for this step?" ->
  {{"intent":"step_items_request","item_scope":"parts","part_action":"reference","reference_relation":"alternative_to_previous","step_scope":"selected_step","target":"","candidates":[],"exclude_labels":[]}}
- "Can you get the other part required for this step?" ->
  {{"intent":"step_items_request","item_scope":"parts","part_action":"fetch","reference_relation":"alternative_to_previous","step_scope":"selected_step","target":"","candidates":[],"exclude_labels":[]}}
- "Can you get one of them?" after multiple parts were presented ->
  {{"intent":"part_reference","part_action":"fetch","reference_relation":"member_of_recent_set","step_scope":"","target":"AMBIGUOUS","candidates":[],"exclude_labels":[]}}
- After the gear rod was resolved, "Can you bring it to me?" ->
  {{"intent":"part_reference","part_action":"fetch","reference_relation":"same_as_previous","step_scope":"","target":"","candidates":[],"exclude_labels":[]}}
- "Can you get a part for me?" with a selected step ->
  {{"intent":"step_items_request","item_scope":"parts","part_action":"fetch","step_scope":"selected_step","target":"","candidates":[],"exclude_labels":[]}}
- "Did you give me the tool necessary for this step?" ->
  {{"intent":"step_items_request","item_scope":"tools","part_action":"status","step_scope":"selected_step","target":"","candidates":[],"exclude_labels":[]}}
- "What tools do I need for this step?" ->
  {{"intent":"step_items_request","item_scope":"tools","part_action":"reference","step_scope":"selected_step","target":"","candidates":[],"exclude_labels":[]}}
- "Can you get the tool needed for this step?" ->
  {{"intent":"step_items_request","item_scope":"tools","part_action":"fetch","step_scope":"selected_step","target":"","candidates":[],"exclude_labels":[]}}
- "Can you get me the wrench?" ->
  {{"intent":"part_reference","part_action":"fetch","step_scope":"","target":"BIT_WRENCH","candidates":[],"exclude_labels":[]}}
- With a fastening step focused, "Can you get me the ranch?" ->
  {{"intent":"part_reference","part_action":"fetch","step_scope":"","target":"BIT_WRENCH","candidates":[],"exclude_labels":[]}}
- "Can you get me the bearing?" ->
  {{"intent":"part_reference","part_action":"fetch","step_scope":"","target":"BEARING","candidates":[],"exclude_labels":[]}}
- "Where is the bearing?" ->
  {{"intent":"part_reference","part_action":"reference","step_scope":"","target":"BEARING","candidates":[],"exclude_labels":[]}}
- With the Row 1 gear-rod step focused, "Can you give me the gear necessary for this step?" ->
  {{"intent":"part_reference","part_action":"fetch","step_scope":"","target":"GEAR_ROW1_LEFT","candidates":[],"exclude_labels":[]}}
- "Can you get me the Row 1 kit?" ->
  {{"intent":"part_reference","part_action":"fetch","step_scope":"","target":"ROW1_KIT","candidates":[],"exclude_labels":[]}}
- "Can you highlight the screwdriver for Row 1?" ->
  {{"intent":"part_reference","part_action":"reference","reference_relation":"direct","step_scope":"","target":"BIT_SCREWDRIVER","candidates":[],"exclude_labels":[]}}
- "Can you highlight the bit for Row 1?" ->
  {{"intent":"part_reference","part_action":"reference","reference_relation":"direct","step_scope":"","target":"H5_HEX_BIT","candidates":[],"exclude_labels":[]}}
{clarification}{reference_context}CURRENT USER UTTERANCE:
{text}
"""
    ...
    def apply_policy_response(self, question: str, label: str, spoken: str) -> None:
        ...
        if not question:
            return
        if label == "STEP_ITEMS":
            resolved = "Resolved request: physical items required by the step"
        else:
    ...
    def _normalize_part_label(self, raw: str) -> str:
        ...
        bearing = re.fullmatch(r"BEARING_ROW([1-4])_(LEFT|RIGHT)", canonical)
        if bearing:
            return f"BEARING_ROW{bearing.group(1)}"
        pin = re.fullmatch(r"PIN_ROW([1-4])_(LEFT|RIGHT)", canonical)
        if pin:
            return f"PIN_ROW{pin.group(1)}"
    ...
    def _parse_intent_envelope(self, raw: str) -> dict[str, object]:
        ...
        if intent == "step_items_request":
            ...
            allowed_actions = {"reference", "fetch", ""}
            if item_scope == "tools":
                allowed_actions.add("status")
    ...
    def build_inline(self) -> None:
        ...
        # Study hardware/model initialization is intentionally synchronous:
        # Transformers import, checkpoint reads, and GPU placement all finish
        # on the main thread before inference work is allowed to begin.
        self._worker.start(load_model_in_current_thread=True)
            ...
            if kind == "status":
                ...
                if intent in {"part_reference", "step_items_request"}:
                    ...
                    if (intent == "step_items_request"
                        ...
                        candidate_prompt = f"""\
TASK-AWARE CANDIDATE RESOLUTION
The user asked: {question!r}
Requested item scope: {item_scope or 'all'}.
Using the CURRENTLY SELECTED STEP context, predict the physical pegboard
parts/tools needed by that step. Predict the complete set; never stop after the
first object. Return ONLY this exact flat JSON schema:
{{"intent":"part_reference","part_action":"reference","reference_relation":"direct","step_scope":"selected_step","target":"ALLOWED_LABEL|AMBIGUOUS","candidates":["ALLOWED_LABEL"],"exclude_labels":[]}}
Do not wrap it inside a `part_reference` key. If the complete set
contains exactly one object, put its exact allowed label in target. If the
complete set contains multiple objects, set target to AMBIGUOUS and list every
predicted exact allowed label in candidates. Do not return step_items_request
and do not use Python/graph truth to construct the candidate list.
Return only independently selectable physical objects on the pegboard:
- Never return a label ending in `_ASSEMBLY`; those objects already exist on
  the gearbox and are not retrievable pegboard objects.
- Bearings, screws, and wooden pins are physically supplied inside the row's
  single `ROW1_KIT`, `ROW2_KIT`, `ROW3_KIT`, or `ROW4_KIT` object. Return that
...
"""
                        ...
                        if self._worker.submit(
                            ...
                            _print_console_block(
                                "VLM CANDIDATE REQUEST", question)
                            continue
                    if intent == "part_reference":
                    ...
                    answer = (
                        "Please request a physical part or tool needed for the "
                        "current step." if intent == "out_of_scope" else
                        "I could not understand that reliably. Please rephrase "
                        "your part or tool request.")
                    _print_console_block(
                        "VLM INTENT",
                        "out_of_scope" if intent == "out_of_scope"
                        else f"invalid\nraw: {payload}",
                    )
            ...
            elif kind == "task_candidates":
                question = self._pending_question
                envelope = self._parse_intent_envelope(payload)
                # Qwen sometimes wraps an otherwise valid task-candidate
                # result as {"part_reference": {...}} despite the requested
                # flat schema. Accept only that narrow, structurally equivalent
                # wrapper; ordinary intent parsing remains strict.
                if envelope.get("intent") == "invalid":
                    try:
                        wrapped = json.loads(payload)
                        inner = wrapped.get("part_reference")
                        if isinstance(inner, dict):
                            flattened = {
                                "intent": "part_reference",
                                "part_action": "reference",
                            ...
                            envelope = self._parse_intent_envelope(
                                json.dumps(flattened))
                    except (json.JSONDecodeError, TypeError, AttributeError):
                        pass
                if envelope.get("intent") == "part_reference":
                    label = str(envelope["target"])
                    part_action = self._carry_part_clarification_action(
                        label, self._pending_task_candidate_action)
                    self._part_events.put({
                        "text": question,
                        "label": label,
                        "part_action": part_action,
                    ...
                    _print_console_block(
                        "VLM TASK CANDIDATES",
                        f"{label}\ncandidates: "
                        f"{envelope.get('candidate_labels', [])}\nraw: {payload}")
                else:
                    answer = "I could not predict the required physical objects."
                    self._answer_events.put({
                        "text": question, "answer": answer, "intent": "invalid"})
                    _print_console_block("VLM TASK CANDIDATES", f"invalid: {payload}")
                self._pending_question = ""
                self._pending_part_text = ""
