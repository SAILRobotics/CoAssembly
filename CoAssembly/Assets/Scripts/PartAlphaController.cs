...
public class PartAlphaController : MonoBehaviour
{
    ...
    private void Apply(PartEntry p)
    {
        ...
        if (!_originals.TryGetValue(p.renderer, out Material[] originals) || originals == null)
        {
            originals = p.renderer.sharedMaterials;
            _originals[p.renderer] = originals;

            var colors = new Color[originals.Length];
            for (int i = 0; i < originals.Length; i++)
                colors[i] = GetColor(originals[i]);
            _baseColors[p.renderer] = colors;
        }

        if (p.alpha >= OpaqueThreshold)
        {
            p.renderer.sharedMaterials = originals;
            return;
        }

        if (!_instances.TryGetValue(p.renderer, out Material[] mats) || mats == null)
        {
            mats = new Material[originals.Length];
            for (int i = 0; i < originals.Length; i++)
                mats[i] = new Material(transparentTemplate)
                    { name = transparentTemplate.name + "_" + p.renderer.name + "_" + i };
            _instances[p.renderer] = mats;
        }

        var bases = _baseColors[p.renderer];
        for (int i = 0; i < mats.Length; i++)
        {
            Color b = bases[i];
            SetColor(mats[i], new Color(b.r, b.g, b.b, p.alpha));
        }
