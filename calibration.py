...
POSES_EYE_IN_BASE = [
    [-115.49, -36.82, 107.89, -128.56, -47.32, 47.12],
...
POSE_REFINE_APPROACH = [-129.15, -48.28, 103.25, -150.27, -90.98, 46.58]  # hover 200 mm above target
POSE_REFINE_TARGET   = [-129.15, -31.04, 103.85, -163.10, -91.07, 46.61]
...
if __name__ == "__main__":
    ...
    if args.phase == "record-poses":
        ...
        cal.move_to_marker_center(marker_id=51, z_offset_m=0.175, speed=0.3, accel=0.3)
        time.sleep(1.0)
        # Step 5: move above world origin (marker 0 = [0,0,0] in world frame): lift → XY → descend
        ...
        # Step 5b: move XY to above origin at lifted height
        p_xy = np.array([0.0, 0.0, p_lift[2]])
        cal.robot_ctrl.moveL([*(T_bw @ np.append(p_xy, 1.0))[:3], *ori], 0.3, 0.3)
        # Step 5c: descend to hover height
        p_hover = np.array([0.0, 0.0, 0.171600])
