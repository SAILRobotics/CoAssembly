...
def run(quest_ip: str, anchor_marker_id: int, pegboard_marker_id: int,
    ...
    if _PYBULLET_AVAILABLE:
        if simulation:
            ...
            # T_tcp_handcam hardcoded from phase2_results.npz calibration
            ...
            # Desk camera hidden below scene (skipped in simulation mode)
            ...
            if key == 27:
                ...
                if cam.camera_T is None:
                    ...
                    if anchor_ok:
                        ...
                        haptic_lock_sent = False
                        _last_proximity_relock = _now
                        # Marker 100 IS the scene origin — calibration is in marker-100 frame
                        if pb_scene is not None:
                            pb_scene.set_scene_origin(np.eye(4))
                            pb_scene.update_wall(np.eye(4))
                    ...
                    # Marker 100 does NOT need to be visible here.
